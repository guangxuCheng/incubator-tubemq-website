...
总的来说，Source Release是Apache关注的重点，也是发布的必须内容；而Binary Release是可选项，TubeMQ可以选择是否发布二进制包到Apache仓库或者发布到Maven中央仓库。
...
```shell
➜  ~ svn co https://dist.apache.org/repos/dist/dev/incubator/tubemq tubemq-dist-dev
...
```shell
➜  ~ svn co https://dist.apache.org/repos/dist/release/incubator/tubemq tubemq-dist-release
...
➜  tubemq-dist-release ~ svn ci -m "add gpg key for YOUR_NAME"
```
...
```shell
cd /tmp/apache-tubemq-${release_version}-${rc_version} # 进入源码包目录
mvn deploy -Papache-release -DskipTests  # 开始上传
```
...
> 第一次做发布的人需要做这个操作，KEYS主要是让参与投票的人在本地导入，用来校验sign的正确性

```shell
cd /tmp/tubemq-dist-dev
(gpg --list-sigs <your name> && gpg --armor --export <your name>) >> KEYS
svn add KEYS
svn commit -m 'add gpg key for {YOUR_NAME}'# 提交至svn远程服务器
```

### 4.3.3 添加public key到KEYS文件并提交到SVN仓库
```shell
cd /tmp/tubemq-dist-dev
mkdir ${release_version}-${rc_version} #创建版本目录
cp /tmp/apache-tubemq-${release_version}-${rc_version}/*tar.gz* ${release_version}-${rc_version}/#将3.5中生成的签名文件及tar包拷贝到此处
svn status # 检查svn状态
svn add ${release_version}-${rc_version} # 添加到svn版本
svn status # 检查svn状态svn ci -m
svn commit -m 'prepare for ${release_version} ${rc_version}'# 提交至svn远程服务器
```
svn delete svn://dist.apache.org/repos/dist/dev/incubator/tubemq/0.3.0-incubating-RC0

### 4.4 关闭Maven的staging仓库

此步骤为发布2.7.0及以上版本必须要的步骤。在此之前请先确保所有的artifact都是ok的。

1. 登录http://repository.apache.org ,点击左侧的Staging repositories，
2. 搜索TubeMQ关键字，选择你最近上传的仓库
3. 点击上方的Close按钮，这个过程会进行一系列检查，
4. 检查通过以后, 在下方的Summary标签页上出现一个连接，请保存好这个链接，需要放在接下来的投票邮件当中。链接应该是类似这样的: https://repository.apache.org/content/repositories/orgapachetubemq-1605

WARN: 请注意点击Close可能会出现失败，通常是网络原因，只要重试几次就可以了。可以点击Summary旁边的Activity标签来确认。

## 5. 进入投票
> TubeMQ仍旧在孵化中，需要进行两次投票，
  1. TubeMQ社区投票，发邮件至：dev@tubemq.apache.org
  2. incubator社区投票，发邮件至：general@incubator.apache.org
...
Hello Apache TubeMQ PPMC and Community,
    ...
    https://repository.apache.org/content/repositories/orgapachetubemq-xxxx

    Artifacts were signed with the 57EC647A key which can be found in:
...
```html
标题：[VOTE] Release Apache TubeMQ (Incubating) ${release_version}
...
Hello Incubator Community,
    ...
    Apache TubeMQ (Incubating) version ${release_version}.
    ...
    Release notes:
    • https://github.com/apache/incubator-dlab/releases/tag/2.3.0
...
Thanks,
The Apache TubeMQ (Incubating) Team

```

#### 5.2.2 宣布投票结果模板
```html
标题：[RESULT][VOTE] Release Apache TubeMQ ${release_version}

内容：
Hi all

Thanks for reviewing and voting for Apache TubeMQ (Incubating) ${release_version}
...
all IPMC helped us to review and vote for µ's first release. I will
...
#### 6.2.3. 检查gpg签名
  - 导入公钥
  ```shell
  curl https://dist.apache.org/repos/dist/dev/dubbo/KEYS >> KEYS # 下载KEYS
  gpg --import KEYS # 导入KEYS到本地
  gpg —edit-key {RM_NAME}
  ```
  - 然后使用如下命令检查签名
  ```shell
  for i in *.tar.gz; do echo $i; gpg --verify $i.asc $i ; done
...
#### 6.2.4. 检查源码包的文件内容
  ...
  解压缩apache-tubemq-${release_version}-src.tar.gz，进行如下检查:
...
#### 6.2.5. 检查二进制包
  解压缩apache-tubemq-server-${release_version}-src.tar.gz,
